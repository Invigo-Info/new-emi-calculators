...
function setupEventListeners() {
    ...
    const inputs = [
        { input: 'propertyValue', slider: 'propertyValueSlider', min: 500000, max: 100000000 },
...
function switchTenureUnit(unit) {
    ...
    if (unit === 'months') {
        ...
        if (previousUnit === 'years') {
            ...
            tenureValueInput.value = Math.max(12, Math.min(240, monthsValue));
        }
        
        // Update slider range for months (12 to 240 months = 1 to 20 years)
        tenureSlider.min = 12;
        tenureSlider.max = 240;
        ...
        tenureSlider.value = Math.max(12, Math.min(240, inputValue));
        
        // Update input constraints
        tenureValueInput.min = 12;
        tenureValueInput.max = 240;
        ...
        if (previousUnit === 'months') {
            ...
            tenureValueInput.value = Math.max(1, Math.min(20, yearsValue));
        }
        
        // Update slider range for years (1 to 20 years)
        tenureSlider.min = 1;
        tenureSlider.max = 20;
        tenureSlider.step = 1;
        ...
        tenureSlider.value = Math.max(1, Math.min(20, inputValue));
        
        // Update input constraints
        tenureValueInput.min = 1;
        tenureValueInput.max = 20;
...
function calculateAndUpdate() {
    ...
    // Make API call
    fetch('/calculate-loan-against-property-emi', {
        method: 'POST',
        headers: {
...
function shareLink() {
    ...
    if (navigator.share) {
        navigator.share({
            ...
            text: `Check out my loan against property calculation: Property Value: ${formatCurrency(propertyValue)}`,
            url: shareUrl
        });
...
function generatePrintableContent() {
    ...
    return `
        ...
        <html>
        <head>
            <title>Loan Against Property EMI Calculation</title>
            <style>
        ...
        <body>
            ...
            <div class="calculation-details">
                <table class="result-table">
                    <tr><th>Property Value</th><td>${formatCurrency(propertyValue)}</td></tr>
...
function loadFromUrlParameters() {
    ...
    if (urlParams.get('propertyValue')) {
        document.getElementById('propertyValue').value = urlParams.get('propertyValue');
        document.getElementById('propertyValueSlider').value = urlParams.get('propertyValue');
    }
