...
function setupEventListeners() {
    ...
    const inputs = [
        { input: 'vehiclePrice', slider: 'vehiclePriceSlider', min: 200000, max: 50000000 },
        { input: 'interestRate', slider: 'interestRateSlider', min: 7, max: 16 }
    ];
...
function switchTenureUnit(unit) {
    ...
    if (unit === 'months') {
        ...
        // Update slider range for months (12 to 84 months = 1 to 7 years)
        tenureSlider.min = 12;
        tenureSlider.max = 84;
        ...
        tenureSlider.value = Math.max(12, Math.min(84, inputValue));
        ...
        if (previousUnit === 'months') {
            ...
            tenureValueInput.value = Math.max(1, Math.min(7, yearsValue));
        }
        
        // Update slider range for years (1 to 7 years)
        tenureSlider.min = 1;
        tenureSlider.max = 7;
        tenureSlider.step = 1;
        
        // Set slider value to match the converted input value
        const inputValue = parseFloat(tenureValueInput.value) || 5;
        tenureSlider.value = Math.max(1, Math.min(7, inputValue));
...
function calculateAndUpdate() {
    ...
    const tenureValue = parseFloat(document.getElementById('tenureValue').value) || (currentTenureUnit === 'years' ? 5 : 60);
    ...
    const loanAmount = vehiclePrice;
    
    // Make API call
    fetch('/calculate-commercial-vehicle-emi', {
        method: 'POST',
        headers: {
...
function downloadPDF() {
    const vehiclePrice = parseFloat(document.getElementById('vehiclePrice').value) || 0;
    const tenureValue = parseFloat(document.getElementById('tenureValue').value) || 5;
    const interestRate = parseFloat(document.getElementById('interestRate').value) || 8.5;
...
function shareLink() {
    ...
    if (navigator.share) {
        navigator.share({
            title: 'Commercial Vehicle EMI Calculator',
            text: `Check out my commercial vehicle loan calculation: Vehicle Price: ${formatCurrency(vehiclePrice)}`,
            url: shareUrl
        });
...
function generatePrintableContent() {
    ...
    return `
        ...
        <html>
        <head>
            <title>Commercial Vehicle EMI Calculation</title>
            <style>
        ...
        <body>
            <div class="header">
                <h1>Commercial Vehicle EMI Calculator</h1>
                <p>Generated on ${new Date().toLocaleDateString()}</p>
            </div>
            <div class="calculation-details">
                <table class="result-table">
                    <tr><th>Vehicle Price</th><td>${formatCurrency(vehiclePrice)}</td></tr>
...
function loadFromUrlParameters() {
    ...
    if (urlParams.get('vehiclePrice')) {
        document.getElementById('vehiclePrice').value = urlParams.get('vehiclePrice');
        document.getElementById('vehiclePriceSlider').value = urlParams.get('vehiclePrice');
    }
