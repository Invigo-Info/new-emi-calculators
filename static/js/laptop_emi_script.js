...
function setupEventListeners() {
    ...
    const inputs = [
        { input: 'laptopPrice', slider: 'laptopPriceSlider', min: 10000, max: 300000 },
        { input: 'downPayment', slider: 'downPaymentSlider', min: 0, max: 80000 },
...
function updateDownPaymentMax() {
    const laptopPrice = parseFloat(document.getElementById('laptopPrice').value) || 0;
    const downPaymentInput = document.getElementById('downPayment');
    const downPaymentSlider = document.getElementById('downPaymentSlider');
    
    // Update max value for down payment to laptop price
    downPaymentInput.max = laptopPrice;
    downPaymentSlider.max = laptopPrice;
    
    // If current down payment exceeds laptop price, adjust it
    const currentDownPayment = parseFloat(downPaymentInput.value) || 0;
    if (currentDownPayment > laptopPrice) {
        downPaymentInput.value = laptopPrice;
        downPaymentSlider.value = laptopPrice;
    }
    ...
    if (sliderLabels) {
        sliderLabels.innerHTML = `
            <span>₹0</span>
            <span>₹${formatCurrency(laptopPrice)}</span>
        `;
    }
}
...
function calculateAndUpdate() {
    ...
    const tenureMonths = parseFloat(document.getElementById('tenureValue').value) || 12;
    const interestRate = parseFloat(document.getElementById('interestRate').value) || 18;
    
    // Make API call
    fetch('/calculate-laptop-emi', {
        method: 'POST',
        headers: {
...
function updateResults(data) {
    document.getElementById('laptopPriceResult').textContent = `₹${formatCurrency(data.laptopPrice)}`;
...
function downloadPDF() {
    ...
    printWindow.document.write(`
        ...
        <html>
        <head>
            <title>Laptop EMI Calculator Results</title>
            <style>
...
function downloadExcel() {
    ...
    const tenureMonths = parseFloat(document.getElementById('tenureValue').value) || 12;
    const interestRate = parseFloat(document.getElementById('interestRate').value) || 18;
    const loanAmount = laptopPrice - downPayment;
    ...
    csvContent += "Laptop EMI Calculator Results\n\n";
    csvContent += "Input Parameters\n";
    csvContent += `Laptop Price,₹${formatCurrency(laptopPrice)}\n`;
    ...
    link.setAttribute("download", "laptop_emi_calculation.csv");
...
function shareLink() {
    const laptopPrice = document.getElementById('laptopPrice').value;
    const downPayment = document.getElementById('downPayment').value;
    const tenureMonths = document.getElementById('tenureValue').value;
    const interestRate = document.getElementById('interestRate').value;
    
    const shareUrl = `${window.location.origin}/laptop-emi-calculator/?laptopPrice=${laptopPrice}&downPayment=${downPayment}&tenure=${tenureMonths}&interestRate=${interestRate}`;
    
    if (navigator.share) {
        navigator.share({
            title: 'Laptop EMI Calculator',
            text: 'Check out this laptop EMI calculation',
            url: shareUrl
        }).then(() => {
...
function generatePrintableContent() {
    ...
    return `
        <div class="header">
            <h1>Laptop EMI Calculator Results</h1>
            <p>Generated on ${new Date().toLocaleDateString()}</p>
        </div>
        
        <div class="results">
            <h2>Calculation Summary</h2>
            <div class="result-item"><strong>Laptop Price:</strong> ₹${formatCurrency(laptopPrice)}</div>
...
function loadFromUrlParameters() {
    ...
    const laptopPrice = urlParams.get('laptopPrice');
    ...
    if (laptopPrice) {
        document.getElementById('laptopPrice').value = laptopPrice;
        document.getElementById('laptopPriceSlider').value = laptopPrice;
    }
