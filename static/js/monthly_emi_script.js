...
function setupEventListeners() {
    ...
    const inputs = [
        ...
        { input: 'tenureMonths', slider: 'tenureMonthsSlider', min: 1, max: 360 }
    ];
...
function updateScheduleTable(data) {
    ...
    if (data.amortizationSchedule && data.amortizationSchedule.length > 0) {
        data.amortizationSchedule.forEach(monthData => {
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${monthData.month}</td>
                <td>₹${formatNumber(monthData.principal)}</td>
                <td>₹${formatNumber(monthData.interest)}</td>
                <td>₹${formatNumber(monthData.monthlyPayment)}</td>
...
function updateSliderLabels() {
    ...
    const tenureSliderLabels = document.querySelector('#tenureMonthsSlider').parentElement.querySelector('.slider-labels');
